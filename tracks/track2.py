...
**Cowork** *(Track 6)* is for acting. Use it when the task involves operating on local files autonomously, such as renaming a folder of documents, converting files, synthesizing a report from scattered local notes, or running a scheduled weekly digest. Cowork reads and writes files on your machine. You define the task and it executes without requiring you to stay in the loop for each step.

**Claude Code** *(Track 7)* is for building. Use it when the task involves writing, running, or modifying code. Claude Code has direct access to your project's file system, reads and edits source files, runs tests, and proposes code changes as diffs you review and approve. Track 7 is intended for engineering staff.

**Claude Design** is for visual creation. Launched in April 2026, Claude Design lets you go from a text description to a prototype, pitch deck, one-pager, or interactive design through conversation. Product managers can sketch feature flows and user journeys as interactive wireframes, then pass the output directly to Claude Code for implementation without a separate design handoff. Designers can rapidly generate a wide range of visual directions to explore, or turn static mockups into shareable interactive prototypes without going through code review. Teams can also onboard Claude Design with their existing codebase and design files so every output automatically uses the right colors, typography, and components. Finished work exports as PDF, PPTX, standalone HTML, or sends directly to Canva for further editing. Claude Design has its own usage tracking separate from your standard chat allocation. For Enterprise users, the feature is off by default and must be enabled by an administrator in organization settings.

[[CLAUDE_DESIGN_IMAGE]]

**Extended capabilities within Claude chat**

**Research** *(Track 4, Lesson 6)* browses multiple web sources and compiles a structured research report. Use it when you need a landscape assessment, policy survey, or competitive overview. Research is the most token-intensive option.

**Connectors** *(Track 5, Lesson 1)* link Claude to the tools your team already uses. With the HubSpot connector active, Claude can pull deal notes directly from your CRM. Connectors stay active in a conversation until you disable them.

**Plugins** *(Track 5, Lesson 2)* extend what Claude can do within a session with domain-specific defaults. The Data plugin enables executing queries and rendering charts. The Legal plugin adds document review reasoning for regulatory work. The Sales plugin adds pipeline reasoning and outreach generation patterns.

**The cost principle**

Every capability layer adds to your usage allocation. A focused prompt alone is lightest. Connectors add retrieval overhead. Plugins add domain tooling. Research is the most intensive. Claude Design draws from its own separate allocation. For staff who hit usage limits frequently, simplifying the tool stack is the most direct fix.
""",
        "quiz": [
            {
                "question": (
                ...
                "options": [
                    ...
                    "Chat is for thinking and drafting, Cowork is for acting autonomously on files, web pages, and scheduled tasks, Claude Code is for building and modifying code in a project codebase",
            ...
            },
            {
                ...
                "options": [
                    ...
                    "Research mode, which browses multiple sources and compiles structured findings",
...
def render_lesson(lesson_id: int) -> bool:
    ...
    def render_concept_part(text):
        # Render the whole segment as a single st.markdown call. Previously
        # this split out inline <img>/<div> elements into separate calls so
        # their HTML would pass through unsafe_allow_html. That worked for
        # rendering, but it produced three stacked Streamlit element
        # containers (text-above, image, text-below), each with its own
        # default vertical spacing — which is the source of the unshrinkable
        # gap around inline images. Rendering as one block keeps all the
        # content inside a single element container, so the image sits flush
        # against surrounding text with only the image's own margin driving
        # the spacing.
        if not text.strip():
            return
    ...
    # Image sentinels render a Streamlit native st.image() widget at the
    # sentinel's location. Using st.image instead of embedded <img> HTML
    # gives predictable spacing because it uses a dedicated widget
    # container rather than paragraph-wrapped markdown.
    _image_sentinels = {
        "[[CLAUDE_DESIGN_IMAGE]]": "assets/claude_design_preview.png",
    }
    _active_sentinel = next((s for s in _diagram_sentinels if s in concept), None)
    _active_image_sentinel = next((s for s in _image_sentinels if s in concept), None)
