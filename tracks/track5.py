...
""",
        ...
        "data_challenge": {
            "scenario": (
                "You are a program manager reviewing PHQ-9 assessment data across 10 sites. "
                "The table below shows the number of assessments sent and the completion rate "
                "at each site. The Data plugin can run calculations and generate charts "
                "directly from this data.\n\n"
                "Write a prompt that asks Claude to:\n"
                "- Calculate the volume-weighted average completion rate across all sites\n"
                "- Identify sites where high assessment volume and low completion rate overlap "
                "(e.g. more than 300 assessments sent and below 75% completion) — these are "
                "the highest-risk combinations\n"
                "- Produce a bar chart of completion rates sorted from lowest to highest, "
            ...
            "rubric": (
                ...
                "1. Asks for the volume-weighted average completion rate — not just a simple "
                "average. The prompt must reference both volume (assessments sent) and "
                "completion rate in the calculation request.\n\n"
                "2. Identifies high-risk sites using numeric thresholds for both volume AND "
                "completion rate — e.g. 'more than 300 assessments and below 75%'. A prompt "
                "that says 'high volume and low completion' without defining the cutoffs "
                "earns partial credit only (15 points). Both thresholds must be explicit "
                "for full marks.\n\n"
                "3. Names a bar chart sorted from lowest to highest (or equivalent ascending "
                "order) with a reference or threshold line at 80% (or similar).\n\n"
                "If any criterion is missing or incomplete, name it specifically in the hint "
                "— e.g. 'Your prompt asks Claude to identify high-risk sites but does not "
                "define what counts as high volume or low completion — Claude will have to "
                "guess where the cutoff is.' Never give generic feedback."
            ),
            "model_answer": (
                "Using the assessment data in the table, calculate the volume-weighted average "
                "completion rate across all 10 sites. Identify any sites where assessments "
                "sent exceeds 300 and completion rate is below 75% — these are the "
                "highest-risk combinations. Generate a bar chart of completion rates sorted "
                "from lowest to highest and add a reference line at 80%."
            ),
            "hints": [
                "A volume-weighted average weights each site's completion rate by how many "
                "assessments it sent — sites with more assessments count more. Make sure your "
                "prompt asks for this specifically, not just an average.",
                "Your prompt needs numeric cutoffs for both volume and completion rate to "
                "identify high-risk sites — e.g. 'more than 300 assessments sent and below "
                "75% completion.' Without numbers, Claude decides what high and low mean.",
            ],
        },
    },
    4: {
